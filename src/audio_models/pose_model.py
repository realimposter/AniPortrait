...
def init_biased_mask(batch_size, n_head, max_seq_len, period):
    def get_slopes(n):
    ...
    mask = mask.unsqueeze(0).unsqueeze(0).repeat(batch_size, n_head, 1, 1) + alibi
    return mask
...
class Audio2PoseModel(nn.Module):
    def __init__(
        ...
        self.biased_mask = None  # Initialize as None
    ...
    def infer(self, input_value, seq_len, id_seed=None):
        batch_size = input_value.size(0)  # Get batch size from input
        if self.biased_mask is None or self.biased_mask.size(0) != batch_size:
            self.biased_mask = init_biased_mask(batch_size, n_head=8, max_seq_len=600, period=1).to(input_value.device)
        ...
        for i in range(seq_len):
            ...
            # Ensure tgt_mask matches the shape of pose_input
            tgt_mask = tgt_mask.unsqueeze(0) if len(tgt_mask.shape) < 4 else tgt_mask
